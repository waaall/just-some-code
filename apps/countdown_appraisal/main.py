...
BackupFolder = APP_DIR / 'data' / 'backup'
FileName = APP_DIR / 'data' / '2024年考核目标软件内容.xlsx'
SettingFileName = APP_DIR / 'resources' / 'settings.json'
IconFileName = APP_DIR / 'resources' / 'branden.ico'
Modifiable_columns = [0,1,2,3,4,5,6,7]
ReminderDays = 7
#-----------安装/打包-----------
#pyinstaller --onedir --windowed --name "项目绩效追踪App" --icon=resources/branden.ico --hidden-import plyer.platforms.win.notification main.py
...
# 读取和处理数据
def load_data():
    df = pd.read_excel(FileName, parse_dates=['要求完成时间'], index_col=0)
...
class SettingsDialog(QDialog):
    ...
    def saveSettings(self):
        ...
        SettingFileName.parent.mkdir(parents=True, exist_ok=True)
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(str(IconFileName)))  # 设置窗口图标
    ...
    def backup_data(self):
        ...
        backup_filename = BackupFolder / f'backup-{formatted_date}.xlsx'
        self.df.to_excel(backup_filename)
        self.info_label.setText(f"数据已备份到 '{backup_filename}'")

    def save_data(self):
        # 保存数据到原文件或新文件
        self.df.to_excel(FileName)  # 保存到原文件，也可以指定新文件名
