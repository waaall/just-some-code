...
class FileWindow(QWidget):
    ...
    def __init_file_option_group(self):
        self.file_option_group = QGroupBox('文件操作')
        layout = QGridLayout()

        self.get_select_file_but = QPushButton('提取选中序号')
        self.get_select_file_but.clicked.connect(self.__get_work_folder_indexs)
        layout.addWidget(self.get_select_file_but, 0,0)

        # lable本用来显示执行结果，后决定传递变量由controler显示
        self.get_select_result = QLabel()
        layout.addWidget(self.get_select_result, 1,0)

        self.file_but2 = QPushButton('file_but2')
        layout.addWidget(self.file_but2, 0,1)
        self.file_but2_result = QLabel()
        layout.addWidget(self.file_but2_result, 1,1)

        self.file_but3 = QPushButton('file_but3')
        # self.file_but3.setToolTip('')
        self.file_but3.resize(self.file_but3.sizeHint())
        # self.file_but3.clicked.connect(self.copyIntimeFile)
        layout.addWidget(self.file_but3, 0,2)
        
        self.file_but3_result = QLabel()
        layout.addWidget(self.file_but3_result, 2,2)
    ...
    def bind_file_but3(self, new_name, slot_func):
        self.file_but3.setText(new_name)
        self.file_but3.clicked.connect(slot_func)

    ## 显示file_but2的操作执行结果
    def set_file_but2_result(self, messages):
        self.file_but2_result.setText(messages)

    ## 显示file_but3的操作执行结果
    def set_file_but3_result(self, messages):
        self.file_but3_result.setText(messages)
