...
WorkFolder = BASE_DIR / "examples"  #样例输入所在文件夹
OutputFolder = BASE_DIR / "output"  #运行结果所在文件夹
InputName = "3"                     #数据文件名
...
if __name__ == '__main__':
    ...
    InitMatrics = pd.read_excel(WorkFolder / f"{InputName}{InputFormat}", engine='openpyxl', header=None, names=OutSubName)
    OutputFolder.mkdir(parents=True, exist_ok=True)
    
    #每列数据计算保存一份
    for col in OutSubName:
        #这列数据转为list，然后调用函数，找出所有幅值（amplitude），并保存
        splitAndFind(InitMatrics[col].tolist(), OutputFolder / f"{InputName}_{col}")
        findAllAmp(InitMatrics[col].tolist(), OutputFolder / f"{InputName}_{col}_不求平均值")
