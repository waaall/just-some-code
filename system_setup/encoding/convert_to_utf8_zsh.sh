...
verify_conversion_results() {
    ...
    for ext in "${FILE_EXTENSIONS[@]}"; do
        if [[ "$name_first" == true ]]; then
            name_cond="-name \"$ext\""
            name_first=false
        else
            name_cond+=" -o -name \"$ext\""
        fi
    done
    
    # 统计不同编码的文件数量
    local find_verify_cmd="find \"$ABSOLUTE_BASE_PATH\" \\( $path_cond \\) -type f \\( $name_cond \\) -exec file -I {} \\;"
    local utf8_count=$(eval "$find_verify_cmd" | grep "charset=utf-8" | wc -l | tr -d ' ')

    # 统计所有需要转换的字符集
    local need_convert_count=0
    for charset in "${TARGET_CHARSETS[@]}"; do
        local count=$(eval "$find_verify_cmd" | grep "charset=$charset" | wc -l | tr -d ' ')
        if [[ $count -gt 0 ]]; then
    ...
    local other_count=$(eval "$find_verify_cmd" | grep -v -E "($exclude_pattern)" | wc -l | tr -d ' ')
    ...
    if [[ $need_convert_count -eq 0 ]]; then
        ...
        echo "未转换成功的文件列表："
        for charset in "${TARGET_CHARSETS[@]}"; do
            eval "$find_verify_cmd" | grep "charset=$charset" | cut -d: -f1
        done
    fi
}
